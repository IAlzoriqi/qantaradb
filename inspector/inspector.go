...
func Inspect(dsn string) (*SchemaInfo, error) {
	...
	// Fetch columns, indexes, FKs for each table
	for i, t := range schema.Tables {
		// 1. Columns
		cols, err := inspectColumns(db, dbName, t.Name)
		if err != nil {
			return nil, fmt.Errorf("failed to inspect columns for %s: %w", t.Name, err)
		}
		schema.Tables[i].Columns = cols

		// 2. Indexes
		indexes, err := inspectIndexes(db, dbName, t.Name)
		if err != nil {
			return nil, fmt.Errorf("failed to inspect indexes for %s: %w", t.Name, err)
		}
		schema.Tables[i].Indexes = indexes

		// 3. Foreign Keys
		fks, err := inspectForeignKeys(db, dbName, t.Name)
		if err != nil {
			return nil, fmt.Errorf("failed to inspect foreign keys for %s: %w", t.Name, err)
		}
